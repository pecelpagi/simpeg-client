...
class DialogContextProvider extends Component {
    ...
    processingSubmitData = async ({ formModel, selectedId, refCollections, onRefreshData }) => {
        const reformatFormModel = {
            ...formModel,
            dateFacingHrd: moment(formModel.dateFacingHrd).format('YYYY-MM-DD'),
            violationDate: moment(formModel.violationDate).format('YYYY-MM-DD'),
            suspensionPeriod: formModel.regarding === 'S' ? moment(formModel.suspensionPeriod).format('YYYY-MM-DD') : null
        }
